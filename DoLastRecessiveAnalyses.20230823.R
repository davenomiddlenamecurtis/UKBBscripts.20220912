...
TestName=sprintf("%s.rec.20230823",Phenotype)
OldResFile=sprintf("AllRec.%s.rec.20230802.summ.txt",Phenotype)
OldRes=data.frame(read.table(OldResFile,header=TRUE,sep="\t",stringsAsFactors=FALSE))
...
for (r in 1:nrow(SummRes)) {
	...
	Gene=SummRes$Gene[r]
	if (Gene %in% OldRes$Gene) {
		SummRes[r,]=OldRes[OldRes$Gene==Gene,]
		next
	}
	print(Gene)
