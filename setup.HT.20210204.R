...
cmd=sprintf("tail -n +2 %s | cut -f 1,%d-%d > ../HT/selfDiagnoses.txt",exomesFile,selfReportFirst+1,selfReportLast+1)
system(cmd)
...
for (r in 1:nrow(ICD10Sources)) {
  cmd=sprintf("%s,%d-%d",cmd,ICD10Sources$First[r]+2,ICD10Sources$Last[r]+1)
}
...
cmd=sprintf("tail -n +2 %s | cut -f 1,%d-%d > ../HT/prescribedRx.txt",exomesFile,prescribedRxFirst+1,prescribedRxLast+1)
system(cmd)
