...
do
	...
	then
		...
		dx cd /
		dx run swiss-army-knife --name "$testName.toDo.$chr" -y --instance-type mem3_hdd2_v2_x4  -imount_inputs=FALSE -iin="/scripts/runGeneSetWithFileList.20231112.sh" -icmd="bash runGeneSetWithFileList.20231112.sh $testName $chr $testName.toDo.$chr.txt $otherNeededFileList"
