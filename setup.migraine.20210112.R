...
cmd=sprintf("tail -n +2 %s | cut -f 1,%d-%d > ../migraine.20210104/selfDiagnoses.txt",exomesFile,selfReportFirst+1,selfReportLast+1)
system(cmd)
selfReport=data.frame(read.table("../migraine.20210104/selfDiagnoses.txt",header=FALSE,sep="\t"))
...
cmd=sprintf("%s > ../migraine.20210104/ICD10Codes.txt",cmd)
system(cmd)

ICD10=data.frame(read.table("../migraine.20210104/ICD10Codes.txt",header=FALSE,sep="\t"))
...
cmd=sprintf("tail -n +2 %s | cut -f 1,%d-%d > ../migraine.20210104/prescribedRx.txt",exomesFile,prescribedRxFirst+1,prescribedRxLast+1)
system(cmd)
prescribedRx=data.frame(read.table("../migraine.20210104/prescribedRx.txt",header=FALSE,sep="\t"))
...
for (c in 2:ncol(migraine)) {
  ...
  write.table(toWrite,sprintf("../migraine.20210104/UKBB.%s.txt",colnames(migraine)[c]),sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
}
write.table(migraine,"../migraine.20210104/UKBB.migraineall.txt",sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
