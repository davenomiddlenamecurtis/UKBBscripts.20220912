...
if (file.exists(SummResFile)) {
	...
	SummRes=data.frame(matrix(NA, ncol = 9, nrow = nrow(SLPs)),stringsAsFactors=FALSE)
	colnames(SummRes)=c("Gene","NumCarriers","ScoreBMISLP","RankScoreBMISLP","CarrierBMISLP","RankScoreObeseSLP","CarrierObeseSLP","PccSLP","KccSLP")
...
for (r in 1:nrow(SummRes)) {
	...
	if (nrow(Carriers)<3) {
		print(sprintf("Fewer than three valid carriers for gene %s", Gene))
		next
	}
