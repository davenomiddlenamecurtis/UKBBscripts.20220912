...
for (c in 2:ncol(T2D)) {
  toWrite=T2D[,c(1,c)]
  write.table(toWrite,sprintf("../T2D.20210104/UKBB.%s.txt",colnames(T2D)[c]),sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
}
write.table(T2D,"../T2D.20210104/UKBB.T2Dall.txt",sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
