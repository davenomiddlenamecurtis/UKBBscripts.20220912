...
# invoke with:
# genes=HERC1; chr=15; model=UKBB.LOF.20230809; testName=$model; gene=$genes
# dx rm /workdir/'*'.$gene.'*'; dx rm /results/$testName/geneResults/'*'.$gene.s'?'o ; dx rm /results/$testName/failed/$gene.failed.txt
# dx cd / ; dx run swiss-army-knife -y --instance-type mem3_hdd2_v2_x4  -imount_inputs=FALSE -iin="/scripts/runGeneLOF.db.20230810.sh"  -icmd="bash runGeneLOF.db.20230810.sh $model $chr $genes "
