...
ICD10=data.frame(read.table("../lipids/ICD10Codes.txt",header=FALSE,sep="\t"))
...
write.table(sexTab,"../UKBB.sex.20201111.txt",sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
# do it this way just to make sure all tabs correct etc.
...
selfReportRx=data.frame(read.table("../lipids/selfReportRx.txt",header=FALSE,sep="\t"))
...
for (c in 2:ncol(hyperlipidaemia)) {
  ...
  write.table(toWrite,sprintf("../lipids/UKBB.%s.20201103.txt",colnames(hyperlipidaemia)[c]),sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
}
