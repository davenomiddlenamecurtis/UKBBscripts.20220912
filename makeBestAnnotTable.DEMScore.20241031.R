...
AllDone=TRUE
commStr=sprintf("dx cd /results/%s/geneResults",NewModel)
...
for (gene in Top[,1]) {
	...
	if (!file.exists(SummFile)) {
		commStr=sprintf("pushd /home/rejudcu/UKBB/dementia.2024/genes; dx download summ.UKBB.dementia.best.annot.20240620.%s; popd",gene)
		if (!file.exists(SummFile)) {
			AllDone=FALSE
		}
	}
...
# next section needs to be rewritten to run on the RAP
if (!AllDone) {
GenesToDo=TopTests
sink(bashScript)
