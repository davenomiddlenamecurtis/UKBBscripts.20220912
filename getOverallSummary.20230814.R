...
OutputFile="overallSummary.20220428.txt"
PrevOutputFile="variantPrevalences.20220428.txt"
...
PrevTab=data.frame(matrix(ncol=5,nrow=length(genes)-1,0))
colnames(PrevTab)=c("Gene",rNames)
...
for (gene in genes) {
	if (gene!="NOTCACNA1G") {
		...
		if (gene=="SETD1A") {  #first one
			AllGenesData=UKBBdata
		} else if (gene!="CACNA1G") {
			AllGenesData=rbind(AllGenesData,UKBBdata)
		}
		N=nrow(UKBBdata)
		PrevTab[c,2]=gene
		PrevTab[c,2]=N
		PrevTab[c,3]=N*100000/nUKBB
		PrevTab[c,4]=SCHEMATable[which(SCHEMATable$Gene==gene),2]*100000/nSCHEMAcase
		PrevTab[c,5]=SCHEMATable[which(SCHEMATable$Gene==gene),3]*100000/nSCHEMAcontrol
	} else {
		UKBBdata=AllGenesData
		N=nrow(UKBBdata)
	}
	SummaryTab[1,c]=N
for (r in 1:nrow(SummaryScheme)){
...
write.table(PrevTab,PrevOutputFile,sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
