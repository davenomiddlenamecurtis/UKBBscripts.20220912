...
SLPs=data.frame(read.table(SummResFile,header=TRUE,stringsAsFactors=FALSE))
print(sprintf("Number of genes with non-zero SLPs = %d",nrow(SLPs)))
SLPs=SLPs[SLPs$NumCarriers>=50,]

print(sprintf("Number of genes with at least 50 compound heterozygotes = %d",nrow(SLPs)))
MLPs=abs(SLPs[,4:7])
MaxMLPs=apply(MLPs,1,max)
Best=SLPs[MaxMLPs>=3,]
