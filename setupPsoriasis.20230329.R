...
cmd=sprintf("tail -n +2 %s | cut -f 1,%d-%d > ../psoriasis.20230329/SelfDiagnoses.txt",exomesFile,SelfReportFirst+1,SelfReportLast+1)
system(cmd)
SelfReport=data.frame(read.table("../psoriasis.20230329/SelfDiagnoses.txt",header=FALSE,sep="\t"))
...
for (r in 1:length(SelfReportedCodes)) {
  Psoriasis$PsoriasisSelf[rowSums(SelfReport==SelfReportedCodes[r], na.rm = TRUE)>0]=1
}
...
cmd=sprintf("%s >../psoriasis.20230329/ICD10Codes.txt",cmd)
system(cmd)
ICD10=data.frame(read.table("../psoriasis.20230329/ICD10Codes.txt",header=FALSE,sep="\t"))
...
for (c in 2:ncol(Psoriasis)) {
  ...
  write.table(toWrite,sprintf("../psoriasis.20230329/UKBB.%s.txt",colnames(Psoriasis)[c]),sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
}
write.table(Psoriasis,"../psoriasis.20230329/UKBB.Psoriasisall.txt",sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
