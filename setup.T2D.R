...
cmd=sprintf("tail -n +2 %s | cut -f 1,%d-%d > ../T2D/selfDiagnoses.txt",exomesFile,selfReportFirst+2,selfReportLast+2)
system(cmd)
selfReport=data.frame(read.table("../T2D/selfDiagnoses.txt",header=FALSE,sep="\t"))
...
cmd=sprintf("%s > ../T2D/ICD10Codes.txt",cmd)
system(cmd)

ICD10=data.frame(read.table("../T2D/ICD10Codes.txt",header=FALSE,sep="\t"))
T2D$T2DICD10=0
...
cmd=sprintf("tail -n +2 %s | cut -f 1,%d-%d > ../T2D/prescribedRx.txt",exomesFile,prescribedRxFirst+2,prescribedRxLast+2)
system(cmd)
prescribedRx=data.frame(read.table("../T2D/prescribedRx.txt",header=FALSE,sep="\t"))
...
for (c in 2:ncol(T2D)) {
  toWrite=T2D[,c(1,c)]
  write.table(toWrite,sprintf("../T2D/UKBB.%s.txt",colnames(T2D)[c]),sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
}
write.table(T2D,"../T2D/UKBB.T2Dall.txt",sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
