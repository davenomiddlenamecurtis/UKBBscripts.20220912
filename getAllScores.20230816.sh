#!/bin/bash 
set -x
# get scores for all genes in a list using an model and phenotype file

if [ .%1 == . ]
then
	echo Usage: $0  model geneListFile otherNeededFiles
	exit
fi

testName=$1
geneList=$2
otherNeededFiles="$3 $4 $5 $6 $7 $8 $9 "
# these will be passed as a single argument but I think that does not matter
...
do
	...
	then
		...
		dx cd /
		dx run swiss-army-knife --name"$testName.toDo.$chr" -y --instance-type mem3_hdd2_v2_x4  -imount_inputs=FALSE -iin="/scripts/runGeneSet.20230816.sh" -icmd="bash runGeneSet.20230816.sh $testName $chr $testName.toDo.$chr.txt $otherNeededFiles"
	fi
done 
